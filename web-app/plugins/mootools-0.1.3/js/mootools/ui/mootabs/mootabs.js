var mootabs = new Class({
	...
	activate: function(tab, skipAnim){
		...
		if($type(tab) == 'string') 
		{
			myTab = $$('#' + this.elid + ' ul li').filter('[name='+tab+']')[0];
			tab = myTab;
		}
		
		if($type(tab) == 'element')
		{
			var newTab = tab.getProperty('name');
	...
	_getContent: function(){
		...
		var tabRequest = new Ajax(this.options.ajaxUrl + '?tab=' + this.activeTitle.getProperty('name'), this.options.ajaxOptions);
		tabRequest.request();
	},
	...
	removeTab: function(title){
		...
		$$('#' + this.elid + ' ul li').filter('[name='+title+']')[0].remove();
